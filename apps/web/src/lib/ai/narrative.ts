...
8. Prioritize by business impact: revenue > risk > efficiency > curiosity.`;
  ...
  try {
    ...
    return {
      ...
      methodology: sections['Methodology'] ?? sections['methodology'] ?? 'Multi-agent DAG pipeline with 50 specialized agents across 7 stages.',
...
function fallbackNarrative(req: NarrativeRequest): NarrativeResponse {
  ...
  return {
    ...
    methodology: 'The Busara v7 pipeline employs a 7-stage Directed Acyclic Graph (DAG) where 50 specialized agents run in topological order with parallelism within each stage.',
