...
export class NarrativeComposerAgent extends Agent {
  ...
  private composeExecutiveSummary(profile: any, insights: any, deps: Record<string, any>): string {
    ...
    return `This analysis examined a dataset of ${rows} records across ${cols} columns, achieving a data quality score of ${quality}%. ` +
      `The IntelliFlow multi-agent pipeline executed 20 specialized agents in parallel, performing profiling, cleaning, statistical analysis, ` +
  ...
  private composeMethodology(profile: any, deps: Record<string, any>): string {
    const parts: string[] = [];
    parts.push('The IntelliFlow v3 pipeline employs a Directed Acyclic Graph (DAG) execution model where 20 specialized agents run in topological order, maximizing parallelism while respecting dependencies.');
  ...
  private composeFullReport(
    ...
    sections.push(`# IntelliFlow Analysis Report\n\nGenerated: ${new Date().toISOString()}\n\n## Executive Summary\n\n${executiveSummary}`);
